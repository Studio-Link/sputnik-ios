...
final class CallsViewController: UITableViewController {
    ...
    @IBAction func unwindForDialCallSegue(_ segue: UIStoryboardSegue) {
        let dialOptionsViewController = segue.sourceViewController as! DialOptionsViewController

        if let destination = dialOptionsViewController.destination {
            startCall(destination: destination)
        }
    }

    @IBAction func unwindForSimulateIncomingCallSegue(_ segue: UIStoryboardSegue) {
        let simulateIncomingCallViewController = segue.sourceViewController as! SimulateIncomingCallViewController

        if let destination = simulateIncomingCallViewController.destination {
            AppDelegate.shared.displayIncomingCall(uuid: UUID(), handle: destination)
        }
    }
    ...
    private func startCall(destination: String) {
        callManager?.startCall(handle: destination)
    }
